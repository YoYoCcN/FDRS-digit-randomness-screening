...
# 输出文件夹
OUTPUT_DIR <- "FDRS_ML_integrated_RealRawData3_results"
...
extract_dataset_features <- function(
  ...
  feature_vec["joint_12_ChiSquare"] <- js$ChiSquare
  feature_vec["joint_12_Pvalue"] <- js$Pvalue
  feature_vec["joint_12_minus_log10P"] <- js$minus_log10P
  feature_vec["joint_12_CramersV"] <- js$CramersV
  feature_vec["joint_12_H_star"] <- js$H_star
  feature_vec["joint_12_KL"] <- js$KL
  feature_vec["joint_12_MaxAbsResidual"] <- js$MaxAbsResidual
  feature_vec["joint_12_NonzeroCategories"] <- js$NonzeroCategories
  feature_vec["joint_12_Pref_same_pair"] <- js$Pref_same_pair
  feature_vec["joint_12_Pref_neat_combo"] <- js$Pref_neat_combo
  feature_vec["joint_12_Pref_sequential_combo"] <- js$Pref_sequential_combo
  
  joint_freq <- joint_res$frequency
  for (cat_i in joint_freq$Category) {
    feature_vec[paste0("joint_12_freq_", cat_i)] <- joint_freq$Proportion[joint_freq$Category == cat_i]
  }
...
generate_irregular_dataset <- function(n = 255) {
  ...
  # 检测目标是小数点后第1、2位，因此异常模式施加在 d1、d2 上
  mode <- sample(c("258", "05", "neat_pair", "same_pair", "mixed_pref"), 1)
  
  if (mode == "258") {
    
    prob <- rep(0.06, 10)
    prob[c(3, 6, 9)] <- c(0.18, 0.22, 0.18)  # 数字2、5、8偏多
    prob <- prob / sum(prob)
    
    d1 <- sample(0:9, n, replace = TRUE, prob = prob)
    d2 <- sample(0:9, n, replace = TRUE, prob = prob)
    
  } else if (mode == "05") {
    
    prob <- rep(0.07, 10)
    prob[c(1, 6)] <- c(0.23, 0.21)  # 数字0、5偏多
    prob <- prob / sum(prob)
    ...
    pair_prob <- c(
      rep(0.06, 6),
      rep((1 - 0.06 * 6) / 100, 100)
    )
    
    pair <- sample(pair_pool, n, replace = TRUE, prob = pair_prob)
    
    d1 <- as.integer(substr(pair, 1, 1))
    d2 <- as.integer(substr(pair, 2, 2))
    ...
    prob1 <- rep(0.075, 10)
    prob1[c(3, 6, 9)] <- c(0.15, 0.18, 0.14)  # 第1位偏好2、5、8
    prob1 <- prob1 / sum(prob1)
    
    prob2 <- rep(0.08, 10)
    prob2[c(1, 6)] <- c(0.18, 0.18)            # 第2位偏好0、5
    prob2 <- prob2 / sum(prob2)
    
    d1 <- sample(0:9, n, replace = TRUE, prob = prob1)
    d2 <- sample(0:9, n, replace = TRUE, prob = prob2)
  }
  
  # 第3、4位保持相对随机，避免训练异常错放到3、4位
  d3 <- sample(0:9, n, replace = TRUE)
  d4 <- sample(0:9, n, replace = TRUE)
...
# ==========================================================
# 13.1 Train and test predictions
# ==========================================================

# ----------------------------------------------------------
# 1）计算 Isolation Forest 的正常参照原始分数
# ----------------------------------------------------------
# if_train_raw 是 Isolation Forest 在“正常训练集”上的原始 anomaly score
# 后续所有 IF 风险分数都用它作为正常参照范围进行校准
if_train_raw <- predict(
  if_model,
  newdata = normal_train_x,
  type = "score"
)

if_train_raw <- as_prob_vector(if_train_raw)
...
rf_train_prob <- predict_rf(
  rf_model,
  X_train
)

elastic_train_prob <- predict_elastic(
  elastic_model,
  X_train
)

svm_train_prob <- predict_svm(
  svm_model,
  X_train
)

if_train_prob <- predict_if_score(
  if_model,
  X_train,
  reference_scores = if_train_raw
)

supervised_train_prob <- rowMeans(
  cbind(
    rf_train_prob,
    elastic_train_prob,
    svm_train_prob
...
ensemble_train_prob <- LAMBDA_SUPERVISED * supervised_train_prob +
  (1 - LAMBDA_SUPERVISED) * if_train_prob
...
rf_test_prob <- predict_rf(
  rf_model,
  X_test
)

elastic_test_prob <- predict_elastic(
  elastic_model,
  X_test
)

svm_test_prob <- predict_svm(
  svm_model,
  X_test
)

if_test_prob <- predict_if_score(
  if_model,
  X_test,
  reference_scores = if_train_raw
)
...
feature_importance_plot_df <- feature_importance %>%
  dplyr::mutate(
    Feature_class = dplyr::case_when(
      ...
      grepl("^joint_12", Feature) ~ "Joint digit",
      grepl("^prog_12", Feature) ~ "Progressive",
